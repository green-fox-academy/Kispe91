...
{
    std::vector <std::string> studentsWithMoreThan4Candies;
    for (int i = 0; i < students.size(); ++i) {
        if (students[i].second.at("candies") > 4) {
            studentsWithMoreThan4Candies.push_back(students[i].first);
        }
    }
    return studentsWithMoreThan4Candies;
}

int sumOfAgeWithLessThanFiveCandies(const std::vector<std::pair<std::string, std::map<std::string, int>>>& students)
{

    int sumOfAgesWithLessThan5Candies = 0;
    for (int i = 0; i < students.size(); ++i) {
        if (students[i].second.at("candies") < 5) {
            sumOfAgesWithLessThan5Candies += students[i].second.at("age");
        }
    }
    return sumOfAgesWithLessThan5Candies;
}

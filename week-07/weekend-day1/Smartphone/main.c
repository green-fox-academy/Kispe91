...
int main() {
    ...
    smartphone_t *smartphones_array = get_smartphones(file_path, lines);

    printf("The %s is the oldest device in the database\n", get_oldest_smartphone(smartphones_array, lines));
    printf("There are %d phones with BIG (>= 15 cm) screen size\n", get_screen_size_count(smartphones_array, lines, BIG));
